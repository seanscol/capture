...
**Version 3.9 · 2026-09-17 — CORRECTIONS ONLY.**

**The changelog and the open-question history are in `CORRECTIONS-ARCHIVE.md`,
which is not copied to the repos.** The v3.7 shrink moved them without checking
what pointed at them, so `SYSTEM.md`'s references to *DECISIONS 3.3* and
*DECISIONS 3.5* resolve to nothing a session can read, and every repo's copy of
this file has a history stopping on 7 September. **Corrected 2026-09-17:** the
changelog is restored below; the corrections archive keeps only corrections
1–18. — versioned as a set with `SYSTEM.md`. Added
...
**3.7 — 2026-09-13.** The security audit, the noticing design, and the first
shrink.

- **§2.8 added: load off his brain is a clinical requirement.** `[SEAN]` His
  neuropsychiatrist's framing makes it a design test rather than a
  preference. Any feature handing him a list to triage has failed.
- **Routine folds into GLOBAL, not the task app.** `[SEAN]` Reversed within
  the week on use evidence: the sequence is a morning and bedtime thing, so
  it belongs with the morning brief, not beside the task list.
- **The second ecosystem is CREATIVE, not "work".** `[SEAN]` Teaching, Tara
  and tutoring are life and stay in the task app. The old name caused the
  confusion it predicted.
- **Phase 2's noticing design**, from his own missed cases — three
  mechanisms, eleven pattern kinds, one observation a day, one weekly digest
  that says nothing when it has nothing. Complaints become standing
  questions and never close on a single "no".
- **Index everything, surface almost nothing.** Location, browsing history
  and the Gmail archive enter a search index the synthesis cannot read.
- **The prompt-injection line is structural**, enforced by two tests, and
  becomes critical when email is connected.
- **The capture service has two callers, not three.** Routine never called
  it; the planning chat recorded three for several versions from a misread
  route name. Per-caller tokens replace the shared secret, with deletion of
  the old one waiting on positive evidence of migration (§2.1).
- **Security audit, and the findings are in `ASSESSMENT.md`** — a new file,
  written for an external reviewer, stating what is wrong as plainly as what
  works. Headline open items: 17 unauthenticated read routes on the medical
  record, key-prefix-only separation on a shared database, unrotatable
  integration-managed credentials, advisory-not-enforcing hooks, and a bus
  factor of one.
- **Corrections 1–18 archived.** First shrink since the constraint was
  written. See below.

**3.6 — 2026-09-09.** Credentials settled, and a silent data-loss default
closed in four repos.

- **§5.6 rewritten.** A *set* of named tokens is correct and is how per-token
  revocation works; a *fallback between them* is not — unset means unset.
  FND's `WIDGET_TOKEN ?? CRON_SECRET` removed. `GLOBAL_TOKEN` recorded
  across FND, routine and global; routine's GET-only-on-a-named-route-list
  is the pattern to copy. `[SEAN]`
- **Bug family (s): a default correct in development and catastrophic in
  production.** `STORAGE_BACKEND ?? "local"` in four repos — writes accepted,
  success reported, disk gone on the next deploy. Fixed in all four: fail at
  module load, positive condition. Two rules fell out: push before moving
  the variable, and when two apps disagree on a constant the one with least
  to lose moves.
- **Bug family (t): a comment describing a hazard instead of a check
  preventing it.** Global's storage module described this exact failure in
  full, directly above the line that caused it. **If you can write the
  comment, you can write the check.**
- **Two practices on testing:** prove your tests catch breakage rather than
  trusting a green run — FND's six-failed-two-passed signature, routine's
  five deliberate breakages; and a subprocess test must assert that a valid
  config succeeds, or an unrelated failure makes the real assertion pass for
  the wrong reason.
- **`DATA_DIR ?? path.join(cwd, "data")` considered and left.** `[SEAN]`
  Same shape one level down, but production can no longer select the
  filesystem adapter without declaring it and the test guard refuses
  non-temp directories, so the default is unreachable rather than hazardous.
  **Recorded as decided, not as unnoticed** — the distinction the corrections
  log exists to preserve.
- **`capture` holds no storage deliberately**, stated in its `CLAUDE.md` so a
  coverage audit reads a decision rather than finding a gap.
